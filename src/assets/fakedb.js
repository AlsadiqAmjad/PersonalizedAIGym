export const userAliceId = "675000000000000000000001";
export const userBobId = "675000000000000000000002";
export const coachChrisId = "675000000000000000000003";
export const adminSarahId = "675000000000000000000004";
...
export const users = [
  ...
  },
  {
    ...
    profile: {
      ...
      gender: "female",
      fitnessLevel: "advanced",
      goals: ["general-fitness", "improve-endurance"],
      availableEquipment: ["full-gym-access"],
      timePerWorkout: 60,
      workoutDaysPerWeek: 5,
      workoutSplit: "custom",
    ...
    preferences: {
      workoutTime: "morning",
      preferredExercises: ["yoga", "running", "strength-training"],
      dislikedExercises: [],
      notifications: {
        email: true,
        push: true,
    ...
    updatedAt: "2025-01-08T08:00:00.000Z",
  },
];
...
// MEMBER HELPER - Get user data by userId
export function getUserData(userId, { users, meals, nutritionPlans, workouts, workoutSchedules }) {
  const normalizeId = (id) => String(id);
  const user = users.find((u) => normalizeId(u._id) === normalizeId(userId));
  ...
  const userMeals = meals.filter((m) => normalizeId(m.userId) === userKey);
  const userPlans = nutritionPlans.filter((p) => normalizeId(p.userId) === userKey);
  const userWorkouts = workouts.filter((w) => normalizeId(w.userId) === userKey);
  const userSchedules = workoutSchedules.filter((s) => normalizeId(s.userId) === userKey);

  // Get today's workout from schedule
  const activeSchedule = userSchedules.find((s) => s.isActive);
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  
  let todaysWorkout = null;
  if (activeSchedule) {
    const todaySchedule = activeSchedule.schedule.find((item) => {
    ...
    if (todaySchedule) {
      todaysWorkout = userWorkouts.find((w) => normalizeId(w._id) === normalizeId(todaySchedule.workoutId));
    }
  }
  ...
  return {
    user,
    meals: userMeals,
    nutritionPlans: userPlans,
    workouts: userWorkouts,
    workoutSchedules: userSchedules,
    todaysWorkout,
    activeSchedule,
  };
}
